...
final class NotebookPageStore: ObservableObject {
    ...
    func insertPage(_ model: NotebookPageModel, at index: Int? = nil, controller: CanvasController? = nil) {
        ...
        if let controller {
            pageControllers[model.id] = controller.id == model.id ? controller : CanvasController(id: model.id)
        } else {
    ...
    func updateModels(_ models: [NotebookPageModel]) {
        ...
        syncControllers()
        if let active = activePageID, pageModels.contains(where: { $0.id == active }) {
            activePageID = active
        } else {
            activePageID = pageModels.first?.id
        }
    }
}
